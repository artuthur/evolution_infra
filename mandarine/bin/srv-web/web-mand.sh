...
Ce script effectue les tâches suivantes :
    ...
    - Création des fichiers de zone pour le domaine "mandarine.iut" et sa reverse DNS, 
    en mode esclave.
    - Ajout du serveur DNS maître pour la synchronisation des zones.
    - Configuration de `named.conf.local`, `db.mandarine.iut`, et `db.10-ptr` 
    pour définir 
    ...
    - Configuration d'Apache pour servir le site web et l'intégration 
    de Rainloop pour la gestion des mails.
